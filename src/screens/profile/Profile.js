            ...
            <div>
                ...
                <div className="profile-Info">
                    <IconButton color="primary" className="profile-picture-body">
                        <img src={this.state.profileData.data.profile_picture} className="profile-img-large" alt={this.state.profileData.data.username}/>
                    </IconButton>
                    <div className="profile-data">
                        <div>
                            ...
                            <div className="profile-stats">
                               <span className="counts-media"> Posts: {this.state.profileData.data.counts.media} </span>
                               <span className="counts"> Follows: {this.state.profileData.data.counts.follows} </span>
                               <span className="counts"> Followed By: {this.state.profileData.data.counts.followed_by} </span>
                            </div>
                            <div className="profile-full-name">
                                {this.state.profileData.data.full_name}
                                <span className="span-fab">
                                    <Fab size="small" color="secondary" className="edit-button" onClick={this.openModelHandler}>
                                        <EditIcon />
                                    </Fab>
                                </span>
                            </div>
                ...
                >
                    <TabContainer>
                            ...
                            <FormControl>
                                ...
                                <Input id="fullName" type="text" fullName={this.state.updated_full_name} onChange={this.inputFullChangeHandler}/>
                            </FormControl>
                            <br /><br />
                ...
                <GridList cellHeight={300} cols={3}>
                    {this.state.pageData.map(data => (
                        <GridListTile className="grid-item" key={"grid" + data.id} style={{ height: 'auto' }} onClick={()=>this.showPostDetails(data)} >
                ...
                >
                    <div className="post-details">
                        <div className="post-image-div">
                            <img src={this.state.postDetails.images.standard_resolution.url} className="post-image" alt="test" /> 
                        </div>
                        <div className="post-user-container">
                            <div className="post-info">
                                ...
                                <Avatar aria-label="recipe">
                                    <img src={this.state.postDetails.caption.from.profile_picture} className="posted-by" alt="test" />
                                </Avatar>
                                }
                                </span>
                                <span className="span-user-name">{this.state.postDetails.user.username}</span>
                            </div>
                            <hr className="horizontal-rule-style"/>
                            <div>
                                {this.state.postDetails.caption.text.split("\n")[0]}
                            </div>
                            ...
                            <div className="div-comments">
                                <div className="likes-div">
                                    <IconButton onClick={() => this.likePost(this.state.postDetails)}>
                                        {
                                    ...
                                    <span className="like-counter">  {this.state.postDetails.likes.count} Likes</span>
                                </div>
                                <div className="comments-show" style={{display: this.state.postDetails.user_commented ? 'block' : 'none' }}>
                                    <span className="comment-username">{this.state.postDetails.user.username}:</span>
                                    <span> {this.state.postDetails.user_comments}</span>
                                </div>
                                <div className="comment-section">
                                    <FormControl>
                                        <InputLabel htmlFor={"inputComment"+this.state.postDetails.id}>Add a comment</InputLabel>
                                        <Input id={"inputComment"+this.state.postDetails.id} type="text" inputcomment={this.state.comment} onChange={this.commentTextChangeHandler}/>
                                    </FormControl>
                                    <Button variant="contained" color="primary" onClick={() => this.AddCommentClickHandler(this.state.postDetails)}>ADD</Button>
                                </div>
